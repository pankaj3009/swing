...
if (length(args)>1 && args[1] == 2) {
  #Derived Trades
  pnl<-data.frame(bizdays=as.Date(mdsubset$date,tz=kTimeZone),realized=0,unrealized=0,brokerage=0)
  cumpnl<-CalculateDailyPNL(futureTrades[complete.cases(futureTrades),],pnl,kFNODataFolder,kBrokerage,per.contract.brokerage = FALSE)
