...
//   NumericVector buyprice=NumericVector::create( as<NumericVector>(all["buyprice"]));
//   NumericVector sellprice=NumericVector::create(as<NumericVector>(all["sellprice"]));
//   NumericVector shortprice=NumericVector::create(as<NumericVector>(all["shortprice"]));
//   NumericVector coverprice=NumericVector::create(as<NumericVector>(all["coverprice"]));   DatetimeVector timestamp=all["date"];
  ...
  for(int i=0;i<nSize;i++){
      ...
      if(true){    
    //if((inlongtrade[i-1]==1)|(inshorttrade[i-1]==1)){//stoploss needs to be evaluated
      //bool newtrade=((inlongtrade[i-1]==1) & (buy[i-1]>0))|((inshorttrade[i-1]==1) & (shrt[i-1]>0));
        // change defnition of newtrade. The line above will consider the original trade to be newtrade.
        // The line below will consider replacement trade to be ALSO be newtrade.
