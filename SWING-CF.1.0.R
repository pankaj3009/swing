...
r <- rollapply(md$aclose, 90, r2)
md$r <- c(rep(NA, nrow(md) - length(r)), r)
AnnualizedSlope = (exp(rollapply(md$aclose, 90, slope)) ^ 252) - 1
md$annualizedslope <-  c(rep(NA,nrow(md) - length(AnnualizedSlope)),AnnualizedSlope)
md$atr_1 <- ATR(md[, c("high", "low", "close")],n=1)[, c("atr")]*100/md$aclose
md$atr_2 <- ATR(md[, c("high", "low", "close")],n=2)[, c("atr")]*100/md$aclose
md$atr_3 <- ATR(md[, c("high", "low", "close")],n=3)[, c("atr")]*100/md$aclose
md <- na.omit(md)
...
signals <- ApplyStop(mdsubset, mdsubset$stoplosslevel,volatilesl = FALSE)
...
futureSignals<-futureTradeSignals(signals,kFNODataFolder,kNiftyDataFolder)
futureTrades<-GenerateTrades(futureSignals)
futureTrades<-futureTrades[order(futureTrades$entrytime),]
for(i in 1:nrow(futureTrades)){
  if(futureTrades$exittime[i]<"2000-01-01"){
    futureTrades$exittime[i]<-strptime(NA,format="%Y-%m-%d",tz=kTimeZone)
  }
}
if(kMoneyManagement){
...
futureTrades$size<-merge(futureTrades,amendedsizedf,by.x="entrytime",by.y="date")$size*kContractSize
#futureTrades$size<-kContractSize*kMaxContracts
futureTrades$underlying<-merge(futureTrades,amendedsizedf,by.x="entrytime",by.y="date")$close

pricediff<-futureTrades$exitprice-futureTrades$entryprice
absprofit<-ifelse(grepl("BUY",futureTrades$trade),pricediff,-pricediff)
futureTrades$percentprofit<-absprofit/futureTrades$underlying
futureTrades$brokerage <-2*kBrokerage
futureTrades$netpercentprofit <-  futureTrades$percentprofit - futureTrades$brokerage


# update option prices for trades today
# update entry prices
out <- which(as.Date(futureTrades$entrytime,tz=kTimeZone) == Sys.Date())
if(length(out)>0){
...
# update exit prices
out <- which(as.Date(futureTrades$exittime,tz=kTimeZone) == Sys.Date()|is.na(futureTrades$exittime))
if(length(out)>0){
  for(o in 1:length(out)){
    index<-out[o]
    newrow <- getPriceArrayFromRedis(9,futureTrades$symbol[index],"tick","close",paste(today, " 09:12:00"))
...
if (kWriteToRedis &  length(args) > 1 && args[1]==4 ) {
  ...
  size <- ifelse(is.na(futureTrades[nrow(futureTrades),'exittime']),futureTrades[nrow(futureTrades),'size'],0)
  strategyside <- ifelse(grepl("BUY",futureTrades[nrow(futureTrades),'trade']), "BUY", ifelse(grepl("SHORT",futureTrades[nrow(futureTrades),'trade']), "SHORT", "AVOID"))
  symbol <- ifelse(nrow(futureTrades>0),futureTrades[nrow(futureTrades),'symbol'],"")
...
if (length(args)>1 && args[1] == 2) {
  #Bootstrap
  trades<-trades[complete.cases(futureTrades),]
  futureTrades<-futureTrades[complete.cases(futureTrades),]
  #  par(mfrow = c(2, 4))
  returns <-
    trades[trades$entrytime >= "2013-04-01" & trades$entrytime <= "2016-03-31" , c("netpercentprofit")]
  derivedreturns <-
    futureTrades[futureTrades$entrytime >= "2013-04-01" & futureTrades$entrytime <= "2016-03-31" , c("netpercentprofit")]
  ...
  margin.requirement = max.exposure*kExchangeMargin
...
if (length(args)>1 && args[1] == 2) {
  ...
  cumpnl<-CalculateDailyPNL(futureTrades[complete.cases(futureTrades),],pnl,kFNODataFolder,kBrokerage,per.contract.brokerage = FALSE,deriv=TRUE)
  ...
  grosspnl<-(futureTrades$exitprice-futureTrades$entryprice)*futureTrades$size
  brokerage<- (futureTrades$exitprice*kBrokerage*futureTrades$size + futureTrades$entryprice*kBrokerage*futureTrades$size)
  futureTrades$grosspnl<-ifelse(grepl("BUY",futureTrades$trade),grosspnl,-grosspnl)
  futureTrades$brokerage<-brokerage
  NetProfit<-sum(futureTrades$grosspnl-futureTrades$brokerage)
  AverageMargin=sum(futureTrades$entryprice*futureTrades$size*futureTrades$bars*kExchangeMargin)/sum(futureTrades$bars)
  ...
  futureTrades$fiscalyear=(2014:2023)[ findInterval(futureTrades$entrytime,fy.tmp) ]
  ...
  fiscal.pnl.recon<-tapply(futureTrades$grosspnl-futureTrades$brokerage,futureTrades$fiscalyear,sum)
  fiscal.margin.num<-tapply(futureTrades$size*futureTrades$entryprice*futureTrades$bars,futureTrades$fiscalyear,sum)
  fiscal.margin.den<-tapply(futureTrades$bars,futureTrades$fiscalyear,sum)
  fiscal.margin<-fiscal.margin.num*kExchangeMargin/fiscal.margin.den
  fiscal.winratio.num<-tapply((futureTrades$grosspnl-futureTrades$brokerage)>0,futureTrades$fiscalyear,sum)
  fiscal.winratio.den<-tapply(futureTrades$grosspnl,futureTrades$fiscalyear,length)
  fiscal.winratio=fiscal.winratio.num/fiscal.winratio.den  
  fiscal.underlying.max<-tapply(futureTrades$underlying,futureTrades$fiscalyear,max)
  ...
  reporting=data.frame(reporting,as.data.frame(fiscal.underlying.max)$fiscal.underlying.max*kExchangeMargin*kContractSize*kMaxContracts)
  colnames(reporting)<-c("Sharpe","WinRatio","NetP&L","ExchangeMargin","BudgetedMargin")
  ...
  reporting$ExchangeMargin<- formatC(reporting$ExchangeMargin,format="d", big.mark=',')
...
if (length(args)>1 && args[1] == 0 )
  # Build Model
{
  seed <- load("random_state_seed.Rdata")
  ...
  data <- na.omit(data)
  trainingdata <- data[data$date >= "2002-04-01" & data$date < kBackTestStartDate,]
  ...
  stopCluster(cl)
  fileName <- paste("fit", "NSENIFTY", "swingc",strftime(kBackTestStartDate,"%Y%m%d"),"Rdata", sep = ".")
  fit$seed <- seed
